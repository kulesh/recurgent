...
1. A failed Specialist delegation MUST produce an `Outcome` with `status: :error` (or runtime-equivalent).
2. Delegation failure MUST NOT force immediate termination of the containing Solver workflow.
...
5. Solver MUST be able to continue subsequent delegations after one failure.
